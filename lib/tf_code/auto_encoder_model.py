...
def inputs(eval_data, leaveout=None, batch_size = None):
    ...
    return data_input.inputs(eval_data=eval_data, data_dir=data_dir,
                             batch_size=batch_size, leaveout=leaveout, data_type=data_type)
...
def inference(images):
    ...
    # conv1
    conv1 = _conv_layer(images,[3,3,depth,96], 'encoder_conv1', 'SAME', True)
    # pool1
    pool1 = tf.nn.max_pool(conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
                           padding='SAME', name='encoder_pool1')
    # conv2
    conv2 = _conv_layer(pool1, [3, 3, 96, 128], 'encoder_conv2', 'SAME', False)

    # conv3
    conv3 = _conv_layer(conv2, [3, 3, 128, 128], 'encoder_conv3', 'SAME', False)

    # pool2
    pool2 = tf.nn.max_pool(conv3, ksize=[1, 2, 2, 1],
                           strides=[1, 2, 2, 1], padding='SAME', name='encoder_pool2')
    # conv4
    conv4 = _conv_layer(pool2, [3, 3, 128, 128], 'encoder_conv4', 'SAME', False)

    # conv5
    conv5 = _conv_layer(conv4, [3, 3, 128, 256], 'encoder_conv5', 'SAME', False)

    # pool3
    pool3 = tf.nn.max_pool(conv5, ksize=[1, 2, 2, 1],
                           strides=[1, 2, 2, 1], padding='SAME', name='encoder_pool3')
    # conv6
    conv6 = _conv_layer(pool3, [3, 3, 256, 256], 'encoder_conv6', 'SAME', False)
    ...
    dec_conv8 = _conv_trans_layer(dec_conv9, [3, 3, 256, 512], 256, 'decoder_conv8', 2, padding='SAME', output_shape=[batch_size, 7, 7, 256])
    ...
    dec_conv1 = _conv_trans_layer(dec_conv2, [3, 3, depth, 96], depth, 'decoder_conv1', 2, padding='SAME', activation=False)
    #
