...
import parse_binary_data_gradient as parse_binary_data
FLAGS = tf.app.flags.FLAGS
tf.app.flags.DEFINE_boolean('use_mag', True, """whether to use magnitude of gradient.""")
...
def read_data(filename_queue, datatype='synthetic', num_tags=17):
  ...
  record_bytes = (label_bytes + image_bytes*3) * 4  # float32 encoding
  ...
  depth_major_x = tf.reshape(tf.slice(record_bytes, [label_bytes], [image_bytes]),
                             [result.height, result.width])
  # print(depth_major_x)

  depth_major_y = tf.reshape(tf.slice(record_bytes, [label_bytes+image_bytes], [image_bytes]),
                             [result.height, result.width])

  depth_major_mag = tf.reshape(tf.slice(record_bytes, [label_bytes + 2*image_bytes], [image_bytes]),
                             [result.height, result.width])

  if FLAGS.use_mag:
    depth_major = tf.pack( [depth_major_x, depth_major_y, depth_major_mag])
  else:
    depth_major = tf.pack([depth_major_x, depth_major_y])
  # print(depth_major)
...
def distorted_inputs(data_dir, batch_size, data_type, num_tags=17):
  ...
  """
  if num_tags == 17:
    filenames = [os.path.join(data_dir, 'grad_thre-10_train_batch_%d.bin' % i)
                 for i in range(0, 8)]
  elif num_tags == 118:
...
def inputs(eval_data, data_dir, batch_size, leaveout = None, min_que = None, num_tags=17, data_type ='synthetic', num_crops = None):
  ...
  if num_tags == 1:
    ...
    if num_tags==118:
      tag_string = '_118tags'
    if eval_data == 'train_eval':
      filenames = [os.path.join(data_dir, 'grad_thre-10_train_batch%s_%d.bin' %( tag_string, i))
                   for i in range(0, 8)]
      num_examples_per_epoch = NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN
      num_preprocess_threads = 16
    elif eval_data == 'test':
      filenames = [os.path.join(data_dir, 'grad_thre-10_val_batch%s_%d.bin' % (tag_string, i))
                   for i in range(0, 2)]
      num_examples_per_epoch = NUM_EXAMPLES_PER_EPOCH_FOR_EVAL
