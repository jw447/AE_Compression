...
tf.app.flags.DEFINE_string('architecture', '5layer','network architecture, could be vgg_16 or alexnet')
...
def eval_once(saver, summary_writer, prob_op, gt_op, summary_op, fc_op):
  """Run Eval once.
  ...
  with tf.Session(config=config) as sess:
    ...
    try:
      ...
      sio.savemat(FLAGS.architecture + '_' + FLAGS.eval_data + '_pred.mat', pred_metrics)

      fc_metrics = dict()
      fc_metrics['label'] = gt_all
      fc_metrics['feature'] = fc_all
      sio.savemat(FLAGS.architecture + '_' + FLAGS.eval_data + '_fc.mat', fc_metrics)
...
def evaluate():
  ...
  with tf.Graph().as_default() as g:
    ...
    # inference model.
    fcs, logits = model.inference_getfc(images, FLAGS.architecture)
    ...
    while True:
      eval_once(saver, summary_writer, prob_op, labels, summary_op, fcs)
      if FLAGS.run_once:
        break
