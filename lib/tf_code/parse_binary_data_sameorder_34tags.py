...
ALL_TAG_FILE = '../data/34tags_meta.txt'
ALL_TAG_META = []
...
def _binaryize_one_list(all_dirs, order_diridx, order_dirname, save_name, i):
    ...
    with open(os.path.join(DATA_PATH_BIN, '%s_%d.bin' % (save_name, i)), 'wb') as f:
        for dir_list_idx, dir_list_idx_i in zip(order_diridx[i], range(0, len(order_diridx[i]))):
            dir_list_name = all_dirs[dir_list_idx]
            assert dir_list_name == str(order_dirname[i][dir_list_idx_i])
            f.write(_binaryize_one_dir(dir_list_name))
...
def processFile():
    ...
    dirs = glob(DATA_PATH + '/*')
    step = 50
    order = sio.loadmat('order_idx.mat')
    order_diridx = order['dir_idxes']
    order_dirname = order['dir_names']
    ...
    #     with open(os.path.join(DATA_PATH_BIN, '34tags_train_batch_%d.bin' % i),'wb') as f:
    #         print('processing 34tags_train_batch_%d.bin' % i)
    #         for j in range(step):
    #             dir_idx = order_diridx[i][j]
    #             dir_name = os.path.join(DATA_PATH, dirs[dir_idx])
    #             assert dir_name == str(order_dirname[i][j])
    #             f.write(_binaryize_one_dir(dir_name))
    #
    #
    # for i in range(num_val_bin_file):
    #     with open(os.path.join(DATA_PATH_BIN, '34tags_val_batch_%d.bin' % i), 'wb') as f:
    #         print('processing 34tags_val_batch_%d.bin' % i)
    #         for j in range(step):
    #             dir_idx = order_diridx[i][j]
    #             dir_name = os.path.join(DATA_PATH, dirs[dir_idx])
    #             assert dir_name == str(order_dirname[i][j])
    ...
    #     delayed(_binaryize_one_list)(dirs, order_diridx, order_dirname, 'train_batch_34tags', i) for i in
    #     range(num_train_bin_file))

    Parallel(n_jobs=num_cores)(
        delayed(_binaryize_one_list)(dirs, order_diridx, order_dirname, 'val_batch_34tags', i+num_train_bin_file) for i in
