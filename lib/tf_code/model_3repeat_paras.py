# based on model.py, but with 3 times more parameters
    ...
    if architecutre == 'vgg_16':
        ...
        with tf.variable_scope('conv1') as scope:
            ...
            for idx in range(depth):
                kernel.append(_variable_with_weight_decay('weights%d'%idx, shape=[7, 7, 1, 64],
                                                      stddev=0.04, wd=0.0))
                biases.append(_variable_on_cpu('biases%d'%idx, [64], tf.constant_initializer(0.0)))
                conv1.append(tf.nn.relu( tf.nn.bias_add(tf.nn.conv2d(inputs[idx], kernel[idx], [1, 1, 1, 1], padding='SAME'), biases[idx])))
            ...
            for idx in range(depth):
                filter_layer1 = tf.transpose(
                    tf.reshape(kernel[idx], ([filter_w, filter_w, filter_d, filter_layer1_width, filter_layer1_height])),
                    [4, 3, 0, 1, 2])
                ...
                filter_to_visual.append(filter_layer1)
                tf.image_summary('filter%d'%idx, filter_to_visual[idx])
        ...
        with tf.variable_scope('conv2') as scope:
            ...
            conv2 = []
            for idx in range(depth):
                kernel.append(_variable_with_weight_decay('weights%d'%idx, shape=[3, 3, 64, 128],
                                                     stddev=0.04, wd=0.0))
                biases.append( _variable_on_cpu('biases%d'%idx, [128], tf.constant_initializer(0.1)))

                conv2.append(
                    tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(pool1[idx], kernel[idx], [1, 1, 1, 1], padding='SAME'), biases[idx])))
        ...
        with tf.variable_scope('conv3') as scope:
            ...
            conv3 = []
            for idx in range(depth):
                kernel.append(_variable_with_weight_decay('weights%d'%idx, shape=[3, 3, 128, 128],
                                                     stddev=0.04, wd=0.0))
                biases.append(_variable_on_cpu('biases%d'%idx, [128], tf.constant_initializer(0.1)))
                conv3.append(
                    tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(conv2[idx], kernel[idx], [1, 1, 1, 1], padding='SAME'), biases[idx])))
        ...
        with tf.variable_scope('conv4') as scope:
            ...
            conv4 = []
            for idx in range(depth):
                kernel.append( _variable_with_weight_decay('weights%d'%idx, shape=[3, 3, 128, 256],
                                                     stddev=0.04, wd=0.0))
                biases.append(_variable_on_cpu('biases%d'%idx, [256], tf.constant_initializer(0.1)))
                conv4.append(
                    tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(pool2[idx], kernel[idx], [1, 1, 1, 1], padding='SAME'), biases[idx])))
        ...
        with tf.variable_scope('conv5') as scope:
            ...
            conv5 = []
            for idx in range(depth):
                kernel.append( _variable_with_weight_decay('weights%d'%idx, shape=[3, 3, 256, 256],
                                                     stddev=0.04, wd=0.0))
                biases.append(_variable_on_cpu('biases%d'%idx, [256], tf.constant_initializer(0.1)))
                conv5.append(
                    tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(conv4[idx], kernel[idx], [1, 1, 1, 1], padding='SAME'), biases[idx])))
        ...
        with tf.variable_scope('conv6') as scope:
            ...
            conv6 = []
            for idx in range(depth):
                kernel.append( _variable_with_weight_decay('weights%d'%idx, shape=[3, 3, 256, 512],
                                                     stddev=0.04, wd=0.0))
                biases.append( _variable_on_cpu('biases%d'%idx, [512], tf.constant_initializer(0.1)))
                conv6.append(
                    tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(pool3[idx], kernel[idx], [1, 1, 1, 1], padding='SAME'), biases[idx])))
        ...
        with tf.variable_scope('conv7') as scope:
            ...
            conv7 = []
            for idx in range(depth):
                kernel.append(_variable_with_weight_decay('weights%d'%idx, shape=[3, 3, 512, 512],
                                                     stddev=0.04, wd=0.0))
                biases.append(_variable_on_cpu('biases%d'%idx, [512], tf.constant_initializer(0.1)))
                conv7.append(
                    tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(conv6[idx], kernel[idx], [1, 1, 1, 1], padding='SAME'), biases[idx])))
        ...
        with tf.variable_scope('conv8') as scope:
            ...
            for idx in range(depth):
                ...
                biases.append( _variable_on_cpu('biases%d'%idx, [512], tf.constant_initializer(0.1)))
                conv8.append(
                    tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(pool4[idx], kernel[idx], [1, 1, 1, 1], padding='SAME'), biases[idx])))
        ...
        with tf.variable_scope('conv9') as scope:
            ...
            for idx in range(depth):
                ...
                biases.append( _variable_on_cpu('biases%d'%idx, [512], tf.constant_initializer(0.1)))
                conv9.append(
                    tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(conv8[idx], kernel[idx], [1, 1, 1, 1], padding='SAME'), biases[idx])))
        ...
        with tf.variable_scope('local11') as scope:
            ...
            for idx in range(depth):
                ...
                dim = reshape[idx].get_shape()[1].value

                weights.append(_variable_with_weight_decay('weights%d'%idx, shape=[dim, 1000],
                                                      stddev=0.04, wd=0.0))
                biases.append(_variable_on_cpu('biases%d'%idx, [1000], tf.constant_initializer(0.1)))
                local11.append( tf.nn.dropout(tf.nn.relu(tf.matmul(reshape[idx], weights[idx]) + biases[idx]), 0.5))
        ...
        with tf.variable_scope('sigmoid_linear') as scope:
            ...
            biases = []
            for idx in range(depth):
                weights.append( _variable_with_weight_decay('weights%d'%idx, [1000, NUM_CLASSES],
                                                      stddev=1/192.0, wd=0.0))
                biases.append(  _variable_on_cpu('biases%d'%idx, [NUM_CLASSES],
                                      tf.constant_initializer(0.0)))
                separate_outpus.append( tf.add(tf.matmul(local11[idx], weights[idx]), biases[idx], name=scope.name))
        ...
        fusion = tf.pack(separate_outpus)
        sigmoid_linear = tf.reduce_mean(fusion, reduction_indices=0)
        ...
        with tf.variable_scope('conv1') as scope:
            ...
            conv1 = []
            for idx in range(depth):
                weights.append( _variable_with_weight_decay('weights%d'%idx, shape=[11, 11, 1, 96],
                                                      stddev=0.01, wd=0.0))
                biases.append( _variable_on_cpu('biases%d'%idx, [96], tf.constant_initializer(0.1)))
                conv1.append(
                    tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(inputs[idx], weights[idx], [1, 4, 4, 1], padding='SAME'), biases[idx])))
                _activation_summary(conv1[idx])
            ...
            filter_to_visual = []
            for idx in range(depth):
                filter_layer1 = tf.transpose(tf.reshape(weights[idx], ([filter_w,filter_w,filter_d, filter_layer1_width, filter_layer1_height])), [4, 3, 0, 1, 2])
                filter_layer1 = tf.split(0, filter_layer1_height, filter_layer1)
                for height_idx in range(0, filter_layer1_height):
        ...
        with tf.variable_scope('conv2') as scope:
            ...
            conv2 = []
            for idx in range(depth):
                weights.append(_variable_with_weight_decay('weights%d'%idx, shape=[5, 5, 96, 192], stddev=0.01, wd=0.0))
                biases.append( _variable_on_cpu('biases%d'%idx, [192], tf.constant_initializer(0.1)))
                conv2.append(
                    tf.nn.relu(
                        tf.nn.bias_add(tf.nn.conv2d(pool1[idx], weights[idx], [1, 1, 1, 1], padding='SAME'), biases[idx])))
                _activation_summary(conv2[idx])
        ...
        with tf.variable_scope('conv3') as scope:
            ...
            conv3 = []
            for idx in range(depth):
                weights.append( _variable_with_weight_decay('weights%d'%idx, shape=[3, 3, 192, 384], stddev=0.01, wd=0.0))
                biases.append( _variable_on_cpu('biases%d'%idx, [384], tf.constant_initializer(0.1)))
                conv3.append(
                    tf.nn.relu(
                        tf.nn.bias_add(tf.nn.conv2d(pool2[idx], weights[idx], [1, 1, 1, 1], padding='SAME'), biases[idx])))
                _activation_summary(conv3[idx])
        ...
        with tf.variable_scope('conv4') as scope:
            ...
            conv4 = []
            for idx in range(depth):
                weights.append( _variable_with_weight_decay('weights%d'%idx, shape=[3, 3, 384, 256], stddev=0.01, wd=0.0))
                biases.append( _variable_on_cpu('biases%d'%idx, [256], tf.constant_initializer(0.1)))
                conv4.append(
                    tf.nn.relu(
                        tf.nn.bias_add(tf.nn.conv2d(conv3[idx], weights[idx], [1, 1, 1, 1], padding='SAME'), biases[idx])))
                _activation_summary(conv4[idx])
        ...
        with tf.variable_scope('conv5') as scope:
            ...
            conv5 = []
            for idx in range(depth):
                weights.append( _variable_with_weight_decay('weights%d'%idx, shape=[3, 3, 256, 256], stddev=0.01, wd=0.0))
                biases.append( _variable_on_cpu('biases%d'%idx, [256], tf.constant_initializer(0.1)))
                conv5.append(
                    tf.nn.relu(
                        tf.nn.bias_add(tf.nn.conv2d(conv4[idx], weights[idx], [1, 1, 1, 1], padding='SAME'), biases[idx])))
                _activation_summary(conv5[idx])
        ...
        with tf.variable_scope('local4') as scope:
            ...
            for idx in range(depth):
                ...
                weights.append(  _variable_with_weight_decay('weights%d'%idx, shape=[dim, 2000],
                                                  stddev=0.04, wd=0.0))
                biases.append( _variable_on_cpu('biases%d'%idx, [2000], tf.constant_initializer(0.1)))

                local4.append(tf.nn.relu(tf.matmul(reshape[idx], weights[idx]) + biases[idx]))
        ...
        with tf.variable_scope('local5') as scope:
            ...
            biases = []
            for idx in range(depth):
                weights.append( _variable_with_weight_decay('weights%d'%idx, shape=[2000, 2000], stddev=0.01, wd=0.0))
                biases.append( _variable_on_cpu('biases%d'%idx, [2000], tf.constant_initializer(0.1)))
                local5.append(tf.nn.dropout(tf.nn.relu(tf.matmul(local4[idx], weights[idx]) + biases[idx]), 0.5))
                _activation_summary(local5[idx])

        if FLAGS.concate_model:
            fusion = tf.pack(local5)
            fusion = tf.transpose(fusion, [1,0,2])
            fusion = tf.reshape(fusion, [batch_size, -1])

        with tf.variable_scope('sigmoid_linear') as scope:
            dim2 = fusion.get_shape()[1].value

            weights = _variable_with_weight_decay('weights', [dim2, NUM_CLASSES],
                                                       stddev=0.01, wd=0.0)
            biases = _variable_on_cpu('biases', [NUM_CLASSES],
                                           tf.constant_initializer(0.1))
            sigmoid_linear = tf.add(tf.matmul(fusion, weights), biases, name=scope.name)
        ...
        #     biases = []
        #     for idx in range(depth):
        #         weights.append(_variable_with_weight_decay('weights%d' % idx, [4000, NUM_CLASSES],
        #                                                    stddev=0.01, wd=0.0))
        #         biases.append(_variable_on_cpu('biases%d' % idx, [NUM_CLASSES],
        #                                        tf.constant_initializer(0.1)))
        #         separate_outpus.append(tf.add(tf.matmul(local5[idx], weights[idx]), biases[idx], name=scope.name))
        #
        #         _activation_summary(separate_outpus[idx])

        # max pooling, or avg pooling across different channels
        # the size of fusion is [num_channel, batchSz, num_dim]
        # if FLAGS.one_model:
        #     print('using one model')
        #     sigmoid_linear = separate_outpus[0]
        # else:
        #     fusion = tf.pack(separate_outpus)
        #     sigmoid_linear = tf.reduce_mean(fusion, reduction_indices=0)
